...
async def set_desc(message: Message, state: FSMContext, session: AsyncSession):
    car_service = get_car_service(
        get_car_repository(session)
    )
...
async def rename_car(message: Message, state: FSMContext, session: AsyncSession):
    user_id = message.from_user.id
    ...
    if new_name.startswith('/'):
        ...
        await car_service.rename_car(car, rename_car)
